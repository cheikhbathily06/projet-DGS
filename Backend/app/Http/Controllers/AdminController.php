...
class AdminController extends Controller
{
    ...
    public function stats()
    {
        ...
        $caTotal = \App\Models\Colis::sum('cout_transport');

        // CA mensuel (6 derniers mois)
        $caMensuel = \App\Models\Colis::selectRaw(
            'DATE_FORMAT(cree_le, "%Y-%m") as mois, SUM(cout_transport) as total'
        )
        ->where('cree_le', '>=', now()->subMonths(6))
        ->groupBy('mois')
        ->orderBy('mois', 'asc')
        ...
        // CA par agent
        $caParAgent = User::where('role', 'agent')
            ->withSum('colisCreees as ca_genere', 'cout_transport')
            ->orderBy('ca_genere', 'desc')
            ->take(5)
            ->get(['id', 'nom', 'prenom']);
