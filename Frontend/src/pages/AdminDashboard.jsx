  ...
  return (
    <Layout>
      ...
      <p className="text-slate-300 mt-1 text-base">
        Vue globale de la plateforme DGS Track.
      </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">
        {[
          ...
          { label: 'Colis livrés', value: data.resume_statuts.livre || 0, color: 'border-green-500' },
          { label: 'Agents', value: data.total_agents, color: 'border-blue-500' },
          { label: 'Clients', value: data.total_clients, color: 'border-purple-500' },
        ].map((kpi) => (
          <div key={kpi.label} className={`bg-white rounded-xl p-5 border-l-4 ${kpi.color}`}>
            <p className="text-sm font-medium text-slate-500">{kpi.label}</p>
            <p className="text-4xl font-bold text-slate-900 mt-2">{kpi.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(value, name) => [value + ' colis', name]}
                  />
                </PieChart>
              </ResponsiveContainer>
        ...
        {/* Graphique barres - performance agents */}
        <div className="bg-white rounded-xl p-6">
      ...
      {/* Lien rapide gestion utilisateurs */}
      <div className="mt-6 bg-white rounded-xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Créez, modifiez ou désactivez les comptes agents et clients.
          </p>
        </div>
        <Link
          to="/admin/users"
